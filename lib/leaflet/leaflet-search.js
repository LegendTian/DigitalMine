...
L.Control.Search = L.Control.extend({
	...
	options: {
		...
		textErr: '没有查到相关信息',	//error message
		textCancel: 'Cancel',		    //title in cancel button		
		textPlaceholder: '输入煤矿名称...',   //placeholder value			
		position: 'topleft',
...
//////end DOM creations
	...
	_defaultFormatData: function(json) {	//default callback for format data to indexed data
	    var propName = this.options.propertyName,
            ...
            propCode = this.options.propertyCode,
			i, jsonret = {};

	    if( L.Util.isArray(propLoc) )
	        for(i in json)
	            jsonret[_getPath(json[i], propName)] = { loc: L.latLng(json[i][propLoc[0]], json[i][propLoc[1]]), code: _getPath(json[i], propCode) };
	    else
	        for(i in json)
	            jsonret[_getPath(json[i], propName)] = { loc: L.latLng(_getPath(json[i], propLoc)), code: _getPath(json[i], propCode) };
...
//	in this way every record can have a free structure of attributes, only 'loc' is required
	...
	_handleSubmit: function() {	//button and tooltip click and enter submit
		...
		else
		{
			...
			else
			{
				...
				var code = this._getCode(this._input.value);

				if(loc===false)
					this.showAlert();
				else
				{
				    this.showLocation(loc, this._input.value, code);
	...
	_getLocation: function(key) {	//extract latlng from _recordsCache
		...
		if( this._recordsCache.hasOwnProperty(key) )
			return this._recordsCache[key].loc;//then after use .loc attribute
		else
			return false;
	},

	_getCode: function (key) {	//extract latlng from _recordsCache

	    if (this._recordsCache.hasOwnProperty(key))
	        return this._recordsCache[key].code;//then after use .loc attribute
	    else
	        return false;
	},
	...
	showLocation: function (latlng, title, code) {	//set location on map from _recordsCache
		var self = this;

		self._map.once('moveend zoomend', function(e) {

            if (self._markerSearch) {
                self._markerSearch.mineCode = code;
...
L.Control.Search.Marker = L.Marker.extend({
	...
	initialize: function (latlng, options) {
		...
		if(options.icon === true)
		    ...
		    options.icon = L.icon({
		        iconUrl: '../lib/leaflet/images/search.gif',
		        iconSize: [50, 50]
